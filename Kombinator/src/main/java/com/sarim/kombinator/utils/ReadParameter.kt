    ...
    constructorParameters.filter { !it.isBoolean && !it.isEnum && it.enumClassDeclaration == null }.forEach { paramInfo ->
        ...
        if (annotationToUse != null) {
            ...
            valuesForParam = when (paramInfo.type) {
                STRING -> readAnnotationArrayArgument(annotationToUse, "allPossibleStringParams", "String", logger) { item, _ -> item as? String }
                INT -> readAnnotationArrayArgument(annotationToUse, "allPossibleIntParams", "Int", logger) { item, _ -> (item as? Number)?.toInt() }
                FLOAT -> readAnnotationArrayArgument(annotationToUse, "allPossibleFloatParams", "Float", logger) { item, _ -> (item as? Number)?.toFloat() }
                DOUBLE -> readAnnotationArrayArgument(annotationToUse, "allPossibleDoubleParams", "Double", logger) { item, _ -> (item as? Number)?.toDouble() }
                LONG -> readAnnotationArrayArgument(annotationToUse, "allPossibleLongParams", "Long", logger) { item, _ -> (item as? Number)?.toLong() }
                BYTE -> readAnnotationArrayArgument(annotationToUse, "allPossibleByteParams", "Byte", logger) { item, _ -> (item as? Number)?.toByte() }
                CHAR -> readAnnotationArrayArgument(annotationToUse, "allPossibleCharParams", "Char", logger) { item, _ -> item as? Char }
                SHORT -> readAnnotationArrayArgument(annotationToUse, "allPossibleShortParams", "Short", logger) { item, _ -> (item as? Number)?.toShort() }
                U_BYTE -> readAnnotationArrayArgument(annotationToUse, "allPossibleUByteParams", "UByte", logger) { item, l ->
                    when (item) {
                        is UByte -> item
                ...
                U_SHORT -> readAnnotationArrayArgument(annotationToUse, "allPossibleUShortParams", "UShort", logger) { item, l ->
                    when (item) {
                        is UShort -> item
                ...
                U_INT -> readAnnotationArrayArgument(annotationToUse, "allPossibleUIntParams", "UInt", logger) { item, l ->
                    when (item) {
                        is UInt -> item
                        is Int -> item.toUInt()
                ...
                U_LONG -> readAnnotationArrayArgument(annotationToUse, "allPossibleULongParams", "ULong", logger) { item, l ->
                    when (item) {
                        is ULong -> item
